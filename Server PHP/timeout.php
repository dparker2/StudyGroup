...
function confirmIPAddress($value) {
  ...
  $q = "SELECT Attempts, (CASE when LastLogin is not NULL and DATE_ADD(LastLogin, INTERVAL 5 MINUTE)>NOW() then 1 else 0 end) as Denied FROM LoginAttempts WHERE IP = '$value'"; 

  $result = mysqli_query($connection, $q);
  if ($result === FALSE) return 0; // redundant, but just in case
...
function addLoginAttempt($value) {
   ...
   $result = mysqli_query($connection, $q);
   if ($result === FALSE) {
	 // first attempt from this ip, create a database entry
